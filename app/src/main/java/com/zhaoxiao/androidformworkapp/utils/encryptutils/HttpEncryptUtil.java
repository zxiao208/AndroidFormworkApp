...
public class HttpEncryptUtil {
	...
	public static String appDecrypt(String appPrivateKeyStr, String content) throws Exception {
		...
		byte[] response = AESUtils.decryptAES(RSAUtil.base642Byte(encryptContent), aesKey);
	...
	public static String serverDecrypt(String content) throws Exception {
		...
		//用AES秘钥解密APP公钥
		SecretKey aesKey = AESUtils.loadKeyAES(new String(aesKeyBytes));
		byte[] appPublicKeyBytes = AESUtils.decryptAES(RSAUtil.base642Byte(encryptAppPublicKeyStr), aesKey);
		//用AES秘钥解密请求内容
		byte[] request = AESUtils.decryptAES(RSAUtil.base642Byte(encryptContent), aesKey);
